...
class CWValidator:
    ...
    def game_paths(self):
        ...
        if os.path.exists(settings_paths_file):
            ...
            self.config = config.read_file(settings_file)
        else:
    ...
    def mapping_validate(self):
        ...
        if not any(file.endswith('.tsv') for file in os.listdir(ATTILA_EXPORT_DIR)):
            print(f'== No .tsv mapping files were found in {ATTILA_EXPORT_DIR} ==')
            ...
            print(f'== Mapping files found in {ATTILA_EXPORT_DIR}! ==')
